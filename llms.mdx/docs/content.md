...
<Callout type="warn">
  **Experimental.** React Native Native is in active development. Core functionality works, but certain language-specific features may be incomplete or unstable. See [Known Issues](#known-issues) below before using in production.
</Callout>
...
* **Async functions** — only synchronous exports (`sync`) are supported currently. Async/Promise-returning functions are not yet implemented.
