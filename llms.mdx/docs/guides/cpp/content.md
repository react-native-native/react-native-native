...
Use `RNA_EXPORT(sync)` to annotate functions. They become importable from JS.
...
Use `RNA_COMPONENT` with a props struct for native views:
...
RNA_COMPONENT(gradientbox, GradientBoxProps)
