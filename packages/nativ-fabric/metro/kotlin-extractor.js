/**
 * kotlin-extractor.js — extracts @rna_export functions and @rna_component
...
function extractKotlinExports(filepath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check for // @rna_export or // @rna_component
    const exportMatch = line.match(/\/\/\s*@rna_export\s*\(?\s*(sync|async)?\s*\)?/);
    const componentMatch = line.match(/\/\/\s*@rna_component/);
