...
function withReactNativeNative(config) {
  ...
  fs.writeFileSync(path.join(projectRoot, '.ferrum/ios-target'), iosTarget);
  fs.writeFileSync(path.join(projectRoot, '.ferrum/android-target'), 'arm64-v8a');
  ...
  // URL: /__ferrum_dylib/{target}/{origName}_{hash}.{ext}
  ...
  function compileOnDemand(target, origName, requestedHash, ext) {
    ...
    const dylibDir = path.join(projectRoot, '.ferrum/dylibs', target);
