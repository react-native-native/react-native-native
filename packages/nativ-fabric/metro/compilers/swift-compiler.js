...
function extractSwiftExports(filepath) {
  ...
  // Match: // @rna_export (or // RNA_EXPORT(sync)) followed by func name(...) -> ReturnType
  const pattern = /\/\/\s*@?rna_export(?:\s*\(\s*(sync|async)\s*\))?\s*\n\s*(?:@_cdecl\s*\([^)]*\)\s*\n\s*)?func\s+(\w+)\s*\(([^)]*)\)\s*(?:->\s*(\S+))?\s*\{/g;
  let match;
  while ((match = pattern.exec(src)) !== null) {
    const [, mode, name, argsStr, retType] = match;
...
`);
  ...
  swiftWrappers += exports.map(fn => {
    ...
    if (retType === 'String') {
      ...
      resultExpr = `return UnsafePointer(strdup(String(result))!)`;
    }
