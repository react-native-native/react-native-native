...
function resolveOnce(projectRoot) {
  ...
  const cacheFile = projectRoot ? path.join(projectRoot, '.ferrum/kotlin-resolve-cache.json') : null;
  if (cacheFile) {
    try {
  ...
  if (projectRoot) {
    _composeJarsDir = path.join(projectRoot, '.ferrum/compose-jars');
    ...
    const ptJar = path.join(projectRoot, '.ferrum/compose-pretransform/compose-pretransform-1.7.0.jar');
...
function compileKotlinDex(filepath, projectRoot) {
  ...
  const buildDir = path.join(projectRoot, '.ferrum/build', `kt_${moduleId}`);
  ...
  if (!isComponent && functions.length === 0) {
    console.warn(`[ferrum] No @rna_export/@rna_component in ${name}.kt`);
    return null;
  }
  ...
  const dexBase = isComponent ? `ferrum_${moduleId}` : moduleId;
...
function generateFunctionWrapper(filepath, functions, className, moduleId) {
  ...
  // Strip @rna_export comments and existing package declaration
...
function compileKotlinComponent(filepath, projectRoot, name, moduleId,
  ...
  if (isCompose) {
    ...
    const wrapperRewrites = {
      'androidx.compose.foundation.layout.Box': 'com.ferrumfabric.compose.Box',
      'androidx.compose.foundation.layout.Column': 'com.ferrumfabric.compose.Column',
      'androidx.compose.foundation.layout.Row': 'com.ferrumfabric.compose.Row',
      'androidx.compose.foundation.layout.Spacer': 'com.ferrumfabric.compose.Spacer',
    };
    userImports = userImports.map(imp => {
      ...
      if (imp.includes('androidx.compose.foundation.layout.*')) {
        return imp + '\nimport com.ferrumfabric.compose.*';
      }
      return imp;
    });
