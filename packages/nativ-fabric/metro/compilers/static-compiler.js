...
function buildRustStatic() {
  ...
  // Shared types (NativeViewHandle, NativeView) come from nativ-core via crate root.
  ...
  // Forward deps from root Cargo.toml, enabling "unified" feature on nativ-core
  let rootDeps = "";
  try {
...
`,
  ...
  // lib.rs: re-export shared types from nativ-core, declare user modules
  const libRs = [
...
function buildSwiftBridges() {
  ...
  for (const filepath of swiftFiles) {
    ...
    if (isComp) {
      ...
      fs.writeFileSync(
        path.join(bridgeDir, `${moduleId}_reg.c`),
        `
typedef void (*NativRenderFn)(void*, float, float, void*, void*);
...
func _nativ_${fn.name}(_ argsJson: UnsafePointer<CChar>) -> UnsafePointer<CChar> {
    ...
    ${resultExpr}
        ...
        .join("\n");
      fs.writeFileSync(
        path.join(bridgeDir, `${moduleId}_bridge.swift`),
        swiftWrappers,
      );

      console.log(
        `[nativ] Swift bridge: ${moduleId} (${exports.length} functions)`,
      );
    }
  }
}
