...
class CppDaemon extends EventEmitter {
  constructor(projectRoot, includePaths) {
    ...
    this.outputDir = path.join(projectRoot, '.ferrum/dylibs');
    this.sdkPath = null;
  ...
  _startWatcher() {
    ...
    const ignore = new Set(['node_modules', 'ios', 'android', '.ferrum', '.git', 'build', 'Pods']);
  ...
  _onFileChange(filepath) {
    ...
    // Only rebuild files that have RNA_EXPORT
    try {
      const src = fs.readFileSync(filepath, 'utf8');
      if (!src.includes('RNA_EXPORT')) return;
    } catch {
      return;
    }
  ...
  _rebuildFile(filepath) {
    ...
    if (exports.length === 0) {
      console.warn(`[ferrum] No RNA_EXPORT functions in ${rel}, skipping dylib`);
