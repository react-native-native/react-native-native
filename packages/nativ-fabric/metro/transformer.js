...
module.exports.transform = async function nativTransform({
  ...
  // Skip ObjC++ on Android (needs ObjC runtime)
  if (isAndroid && filename.endsWith('.mm')) {
    return upstreamTransformer.transform({
      filename: filename.replace(/\.mm$/, '.js'),
      src: 'export default undefined;\n',
      options, ...rest,
    });
  }
  // Swift on Android: compile if Swift Android SDK is available, otherwise skip
  if (isAndroid && filename.endsWith('.swift')) {
    const { isSwiftAndroidAvailable, extractSwiftExports } = require('./compilers/swift-compiler');
    if (!isSwiftAndroidAvailable()) {
      // No SDK — generate .d.ts stub for IDE, return empty module
      if (isDev) {
        try {
    ...
    // SDK available — fall through to compilation below (handled alongside iOS Swift)
  }
  ...
  if (filename.endsWith('.swift')) {
    const { compileSwiftDylib, compileSwiftAndroidSo, extractSwiftExports } = require('./compilers/swift-compiler');
    ...
    // SwiftUI components are iOS-only — skip on Android
    if (isAndroid && isSwiftComponent) {
      return upstreamTransformer.transform({
        filename: filename.replace(/\.swift$/, '.js'),
        src: 'export default undefined;\n',
        options, ...rest,
    ...
    if (isDev) {
      ...
      const ext = isAndroid ? 'so' : 'dylib';
      srcHash = cachedCompile(src, origName, ext, () => {
        if (isAndroid) {
          compileSwiftAndroidSo(filename, projectRoot, { target: buildTarget });
        } else {
    ...
    const fns = exports.map(fn => ({ name: fn.name, async: fn.async, args: fn.args }));
    const _libExt = isAndroid ? 'so' : 'dylib';
    const shimCode = isDev
      ? cppFunctionShim(fns, moduleId, srcHash, moduleId, _libExt)
      : cppFunctionShimProd(fns, moduleId);
