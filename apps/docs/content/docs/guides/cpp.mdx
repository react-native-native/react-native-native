---
title: C++ / ObjC++
description: Write native functions and components in C++ and Objective-C++.
---
...
Use `RNA_EXPORT(async)` for functions that do heavy work or I/O. They return a `Promise` to JS and run on a background thread automatically:
